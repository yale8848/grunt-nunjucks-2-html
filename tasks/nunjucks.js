...
module.exports = function(grunt) {
    grunt.registerMultiTask('nunjucksMutil', `Renders Nunjucks' templates to HTML`, function() {
        // Declare async task
        const completeTask = this.async()
            //
            // Get options and set defaults
            // @note We're using `undefined` to fallback to Nunjucks' default settings
        const options = this.data.options ? this.data.options : this.options({
            paths: '',
        ...
        let _this = (this.data.render ? this.data.render : this);

        if (this.data.render) {
            _this.files = grunt.task.normalizeMultiTaskFiles(_this, this.target);
        ...
        // Get number of files
        const totalFiles = _this.files.length
